...
class TanamanScreen extends StatefulWidget {
  const TanamanScreen({Key? key}) : super(key: key);
...
class _TanamanScreenState extends State<TanamanScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Data Tanaman Petani'),
        leading: IconButton(
  ...
  Future<nanam?> navigateToEntryForm(BuildContext context) async {
    return await Navigator.push(
      ...
      MaterialPageRoute(builder: (BuildContext context) {
        return InputTanaman(nanam(0, '', '', '')); // Create a new Resep object
      }),
    );
  }
  ...
  Future<nanam?> navigateToEditResepForm(
    ...
    return await Navigator.push(
      context,
      MaterialPageRoute(builder: (BuildContext context) {
        return EditTanaman(tanaman: nanam);
      }),
    );
  }
  ...
  void edittanaman(nanam nanam) async {
    ...
    if (result > 0) {
      updateListView();
      print("Edit Tanaman RESULT: $result");
    }
  }
